...
public class NotificationPushService implements MessageListener {
    ...
    @PostConstruct
    public void init() {
        container.addMessageListener(this, new PatternTopic("notify:push:*"));
    }
    ...
    public void onMessage(Message message, byte[] pattern) {
        try {
            ...
            NotificationHistory notif = (NotificationHistory) redisTemplate.getValueSerializer().deserialize(body);
            if (notif != null) {
                pushToSse(notif.getRecipient(), notif);
            }
        } catch (Exception e) {
            log.warn("Failed to process Redis push message", e);
        }
    }
