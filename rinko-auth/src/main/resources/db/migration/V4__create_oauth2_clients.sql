...
COMMENT
ON TABLE oauth2_clients IS 'OAuth2 客户端注册表';
COMMENT
ON COLUMN oauth2_clients.redirect_uris IS '逗号分隔的允许回调URI列表';
COMMENT
ON COLUMN oauth2_clients.grant_types IS '逗号分隔：authorization_code,client_credentials,refresh_token';
COMMENT
ON COLUMN oauth2_clients.scopes IS '逗号分隔的授权范围';
