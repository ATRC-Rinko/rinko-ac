...
COMMENT ON COLUMN file_versions.id IS '主键ID';
COMMENT ON COLUMN file_versions.file_id IS '文件ID（关联 file_metadata）';
COMMENT ON COLUMN file_versions.version IS '版本号';
COMMENT ON COLUMN file_versions.file_size IS '文件大小（字节）';
COMMENT ON COLUMN file_versions.sha256 IS '文件SHA256哈希值';
COMMENT ON COLUMN file_versions.storage_path IS '存储路径';
