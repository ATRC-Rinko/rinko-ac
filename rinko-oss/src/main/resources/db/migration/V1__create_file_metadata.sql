...
COMMENT ON COLUMN file_metadata.original_name IS '原始文件名';
COMMENT ON COLUMN file_metadata.file_size IS '文件大小（字节）';
COMMENT ON COLUMN file_metadata.content_type IS '文件内容类型（MIME）';
COMMENT ON COLUMN file_metadata.sha256 IS '文件SHA256哈希值';
COMMENT ON COLUMN file_metadata.storage_path IS '存储路径';
COMMENT ON COLUMN file_metadata.storage_type IS '存储类型：LOCAL | S3 | OSS';
COMMENT ON COLUMN file_metadata.parent_id IS '父目录ID';
COMMENT ON COLUMN file_metadata.is_directory IS '是否为目录';
COMMENT ON COLUMN file_metadata.current_version IS '当前版本号';
